	...
	void Update () {
        ...
        else
        {
            ...
            if (acabar >= afterTime)
            {
                ...
                ia.GetComponentInParent<IA_Waypoints>().goWaypoint = true;
                ia.transform.LookAt(ia.GetComponentInParent<IA_Waypoints>().waypoints[ia.GetComponentInParent<IA_Waypoints>().waypoint].transform.position); //mirar al siguiente waypoint
